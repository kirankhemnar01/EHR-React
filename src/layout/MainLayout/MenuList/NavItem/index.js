...
import { LightTooltip } from 'ui-component/aida/base';
...
const NavItem = ({ item, level, parentId, sx = {} }) => {
  ...
  const needTooltip = !drawerOpen && level <= 1;

  const verticalLayoutListItemButton = (
    <ListItemButton
      {...listItemProps}
  ...
  const verticalLayoutListItemButtonWithTooltip = needTooltip ? (
    <LightTooltip title={item.title} placement="right">
      {verticalLayoutListItemButton}
    </LightTooltip>
  ) : (
    verticalLayoutListItemButton
  );
  ...
  return (
    <>
      {layout === LAYOUT_CONST.VERTICAL_LAYOUT || (layout === LAYOUT_CONST.HORIZONTAL_LAYOUT && matchDownMd) ? (
        verticalLayoutListItemButtonWithTooltip
      ) : (
        <ListItemButton
          {...listItemProps}
          disabled={item.disabled}
