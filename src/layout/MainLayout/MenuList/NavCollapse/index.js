...
import NavChatsMenuItem from '../nav-chats-menu-item';

import { useSelector } from 'store';
import { LAYOUT_CONST } from 'aida-constants';
...
const NavCollapse = ({ menu, level, parentId }) => {
  ...
  const menus = menu.children?.map((item) => {
    switch (item.type) {
      ...
      case 'chats-menu-item':
        return <NavChatsMenuItem key={item.id} item={item} level={level + 1} parentId={parentId} />;
      case 'item':
  ...
  // consider as selected only when one of it's child is selected
  const isSelected = (menu?.children ?? []).filter((c) => selectedItem.findIndex((id) => id === c.id) > -1).length > 0;
